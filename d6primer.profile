...
function configure_gcal_events() {
	...
	$blocks = array(
	    'gcal_events_private_id_'. $delta => array(
		    ...
		    'gcal_events_block' . $delta			=> $delta,
		    'gcal_events_admin_name' . $delta		=> 'Academic Calendar',
		    'gcal_events_calendar_id' . $delta		=> 'ncsu.edu_507c8794r25bnebhjrrh3i5c4s@group.calendar.google.com',
		    'gcal_events_cache' . $delta			=> $modulePath . '/cache',
			'gcal_events_cache_duration'. $delta	=> '3600',
		    'gcal_events_private_id'. $delta 		=> $private_id,
		    'gcal_events_num_events'. $delta 		=> '5',
		    'gcal_events_today_only'. $delta 		=> '',
		    'gcal_events_dateformat'. $delta 		=> 'j F, Y',
		    'gcal_events_timeformat'. $delta 		=> 'g:ia',
		    'gcal_events_template_event'. $delta 	=> '<div class="gcal_block_event"><span class="element date">#DATE#</span><span class="element time">#TIME#</span><span class="element title">#TITLE#</span><span class="element location">#LOC#</span></div>',			
		    'gcal_events_template_title'. $delta 	=> '<a target="_blank" title="#TITLE#" href="#URL#">#TITLE#</a>',
			'gcal_events_template_desc'. $delta 	=> '#DESC#',
		    'gcal_events_template_date'. $delta 	=> '#DATE#',
			'gcal_events_template_time'. $delta 	=> '#TIME#',
		    'gcal_events_template_loc'. $delta 		=> '#LOC#',
		    'gcal_events_empty'. $delta 			=> 'No events to display',
		    'gcal_events_footer'. $delta 			=> '<a href="http://go.ncsu.edu/drupal-acad-calendar-more-link" target="_blank" title="Subscribe & view all events">Subscribe & view all events</a>',
